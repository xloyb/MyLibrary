...
interface MyHcaptchaProps {
  redirectLink: string; // The link to redirect to after successful captcha verification
}

const MyHcaptcha: React.FC<MyHcaptchaProps> = ({ redirectLink }) => {
  const [token, setToken] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  ...
  return (
    <div>
      <h3 className="font-bold text-lg">Complete the Captcha</h3>
      <form onSubmit={handleSubmit}>
        {/* hCaptcha Component */}
        <HCaptcha
          sitekey={SITEKEY}
